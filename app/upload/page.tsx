...
const formatExpirationLabel = (hours: number): string => {
  if (hours % 24 === 0) {
    const days = hours / 24;
...
export default function UploadPage() {
  ...
  const [expirationHours, setExpirationHours] = useState(
    SHARE_CONFIG.defaultExpirationHours.toString(),
  );
  ...
  const expirationProfile: "standard" | "video" = hasVideoFiles
    ? "video"
    : "standard";
  const expirationOptions = hasVideoFiles
    ? SHARE_CONFIG.videoExpirationOptionsHours
    : SHARE_CONFIG.standardExpirationOptionsHours;

  useEffect(() => {
    const selected = Number.parseInt(expirationHours, 10);
    if (!expirationOptions.includes(selected)) {
      const fallback = hasVideoFiles
        ? SHARE_CONFIG.defaultVideoExpirationHours
        : SHARE_CONFIG.defaultExpirationHours;
      setExpirationHours(String(fallback));
    }
  }, [expirationHours, expirationOptions, hasVideoFiles]);
    ...
    try {
      ...
      const createResponse = await fetch("/api/upload/create", {
        ...
        body: JSON.stringify({
          ...
          expirationHours: Number.parseInt(expirationHours, 10),
          expirationProfile,
        }),
      });

      if (!createResponse.ok) {
        let createErrorMessage = "Failed to create share";
        try {
          const errorData = await createResponse.json();
          createErrorMessage =
            errorData.message || errorData.error || createErrorMessage;
        } catch {
      ...
      for (const file of files) {
        ...
        if (!registerResponse.ok) {
          ...
          throw new Error(
            `Failed to register file ${file.name}: ${errorData.message || errorData.error || "Unknown error"}`,
          );
        }
          ...
          <Container>
            <div className="mx-auto max-w-2xl">
              ...
              >
                ...
                >
                  Upload files, get a link, share with anyone. Files auto-delete
                  after expiration.
                </p>
              </motion.div>
                    ...
                    <div className="space-y-2">
                      ...
                      >
                        <FiClock
                          ...
                          style={{ color: "var(--color-interactive)" }}
                        />
                        Expiration {hasVideoFiles ? "(Video Profile)" : ""}
                      </Label>
                      <Select
                      ...
                      >
                        ...
                        <SelectContent>
                          {expirationOptions.map((hours) => (
                            <SelectItem key={hours} value={String(hours)}>
                              {formatExpirationLabel(hours)}
                            </SelectItem>
                          ))}
                        </SelectContent>
                      </Select>
                      {hasVideoFiles && (
                        <p
                          ...
                          style={{ color: "var(--color-text-tertiary)" }}
                        >
                          Video files selected: expiration options follow video
                          retention policy.
                        </p>
                      )}
                    </div>
            ...
            <div className="grid gap-8 md:grid-cols-3">
              {[
                ...
                },
                {
                  ...
                  description:
                    "Optimized infrastructure delivers blazing-fast uploads and downloads.",
                },
                {
                  icon: FiLink,
                  title: "Simple Sharing",
                  description:
                    "Generate shareable links instantly. No accounts required for recipients.",
                },
              ].map((feature, index) => (
                <motion.div
                  key={feature.title}
